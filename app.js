...
function displayMenuButtons() {
  // creating a new array with no dublicates
  const categories = ["all", ...new Set(menu.map(item => item.category))];
  // creating a new button with th new item.category dynamically
  const categoryBtns = categories.map(function (category) {
    return `<button class="filter-btn" type="button" data-category="${category}">${category}</button>`
  }).join("")
  btnContainer.innerHTML = categoryBtns

  // filtering the button and displayed items by their categories
